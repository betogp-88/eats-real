-- Esquema de Maix (generado desde supabase/empresas/plantilla_empresa.sql con __SCHEMA__ = maix).
-- Ejecutar COMPLETO después de 0004 y luego exponer el esquema "maix" en Settings → Data API.

create schema if not exists maix;
...
create or replace function recalcular_costo_lote(p_lote uuid) returns void language plpgsql set search_path = maix, public as $$
...
create or replace function trg_lote_costos() returns trigger language plpgsql set search_path = maix, public as $$
...
create or replace function trg_lotes_bolsas() returns trigger language plpgsql set search_path = maix, public as $$
...
create or replace function recibir_lote(p_lote uuid) returns void language plpgsql set search_path = maix, public as $$
...
returns void language plpgsql set search_path = maix, public as $$
...
create or replace function despachar_pedido(p_pedido uuid, p_asignaciones jsonb) returns void language plpgsql set search_path = maix, public as $$
...
create or replace function cancelar_pedido(p_pedido uuid) returns void language plpgsql set search_path = maix, public as $$
...
begin
  for t in select unnest(array['productos','maquiladores','ubicaciones','lotes','lote_costos','movimientos_inv','pedidos','pedido_lineas','despachos',
    ...
    execute format('create policy "miembros_todo" on maix.%I for all to authenticated using (public.es_miembro(''maix'')) with check (public.es_miembro(''maix''))', t);
  end loop;
end $$;

grant usage on schema maix to authenticated, anon;
grant select, insert, update, delete on all tables in schema maix to authenticated;
grant select on existencias, ventas_detalle, resultados_mensuales, clientes_resumen, puntos_venta_resumen to authenticated;
revoke execute on all functions in schema maix from public, anon;
