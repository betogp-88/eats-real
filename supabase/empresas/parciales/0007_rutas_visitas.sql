...
drop policy if exists "miembros_todo" on rutas;   create policy "miembros_todo" on rutas   for all to authenticated using (public.es_miembro('__EMPRESA__')) with check (public.es_miembro('__EMPRESA__'));
drop policy if exists "miembros_todo" on visitas; create policy "miembros_todo" on visitas for all to authenticated using (public.es_miembro('__EMPRESA__')) with check (public.es_miembro('__EMPRESA__'));
...
begin
  for t in select unnest(array['gastos','gastos_marketing','lote_costos','lotes','maquiladores','sync_log']) loop
    ...
    execute format('create policy "admins_todo" on __SCHEMA__.%I for all to authenticated using (public.es_admin(''__EMPRESA__'')) with check (public.es_admin(''__EMPRESA__''))', t);
  end loop;
  -- Rutas puede leer lotes (para despachar) pero no escribirlos
  execute 'drop policy if exists "miembros_leen" on __SCHEMA__.lotes';
  execute 'create policy "miembros_leen" on __SCHEMA__.lotes for select to authenticated using (public.es_miembro(''__EMPRESA__''))';
end $$;
